...
def update_convert_button(
	...
	enabled = (
		terms_checked
		and bool(tts_text and tts_text.strip())
		and bool(model_file and str(model_file).strip())
		and bool(tts_voice and str(tts_voice).strip())
	)
...
def tts_tab():
	...
	convert_button = gr.Button(i18n("Convert"), interactive=False)
	...
	terms_checkbox.change(
		fn=update_convert_button,
		inputs=[terms_checkbox, tts_text, model_file, tts_voice],
