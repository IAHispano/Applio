...
def realtime_tab():
    ...
    with gr.Blocks() as ui:
        ...
        model_file.change(
            ...
            fn=(
                (lambda value: change_config(value, "model_path"))
                if not client_mode
                else None
            ),
            inputs=[model_file],
        ...
        index_file.change(
            ...
            fn=(
                (lambda value: change_config(value, "index_path"))
                if not client_mode
                else None
            ),
            inputs=[index_file],
        ...
        f0_method.change(
            ...
            fn=(
                (lambda value: change_config(value, "f0_method"))
                if not client_mode
                else None
            ),
            inputs=[f0_method],
        ...
        embedder_model.change(
            ...
            fn=(
                (lambda value: change_config(value, "embedder_model"))
                if not client_mode
                else None
            ),
            inputs=[embedder_model],
        ...
        embedder_model_custom.change(
            ...
            fn=(
                (lambda value: change_config(value, "embedder_model_custom"))
                if not client_mode
                else None
            ),
        ...
        input_audio_gain.change(
            ...
            fn=(
                (lambda value: change_config(value / 100.0, "input_audio_gain"))
                if not client_mode
                else None
            ),
        ...
        output_audio_gain.change(
            ...
            fn=(
                (lambda value: change_config(value / 100.0, "output_audio_gain"))
                if not client_mode
                else None
            ),
        ...
        monitor_audio_gain.change(
            ...
            fn=(
                (lambda value: change_config(value / 100.0, "monitor_audio_gain"))
                if not client_mode
                else None
            ),
        ...
        vad_enabled.change(
            ...
            fn=(
                (lambda value: change_config(value, "vad_enabled"))
                if not client_mode
                else None
            ),
            inputs=[vad_enabled],
        ...
        cross_fade_overlap_size.change(
            ...
            fn=(
                (lambda value: change_config(value, "cross_fade_overlap_size"))
                if not client_mode
                else None
            ),
        ...
        extra_convert_size.change(
            ...
            fn=(
                (lambda value: change_config(value, "extra_convert_size"))
                if not client_mode
                else None
            ),
        ...
        silent_threshold.change(
            ...
            fn=(
                (lambda value: change_config(value, "silent_threshold"))
                if not client_mode
                else None
            ),
        ...
        pitch.change(
            ...
            fn=(
                (lambda value: change_config(value, "f0_up_key"))
                if not client_mode
                else None
            ),
            inputs=[pitch],
        ...
        index_rate.change(
            ...
            fn=(
                (lambda value: change_config(value, "index_rate"))
                if not client_mode
                else None
            ),
            inputs=[index_rate],
        ...
        volume_envelope.change(
            ...
            fn=(
                (lambda value: change_config(value, "volume_envelope"))
                if not client_mode
                else None
            ),
        ...
        protect.change(
            ...
            fn=(
                (lambda value: change_config(value, "protect"))
                if not client_mode
                else None
            ),
            inputs=[protect],
        ...
        autotune.change(
            ...
            fn=(
                (lambda value: change_config(value, "autotune"))
                if not client_mode
                else None
            ),
            inputs=[autotune],
        ...
        autotune_strength.change(
            ...
            fn=(
                (lambda value: change_config(value, "autotune_strength"))
                if not client_mode
                else None
            ),
        ...
        proposed_pitch.change(
            ...
            fn=(
                (lambda value: change_config(value, "proposed_pitch"))
                if not client_mode
                else None
            ),
            inputs=[proposed_pitch],
        ...
        proposed_pitch_threshold.change(
            ...
            fn=(
                (lambda value: change_config(value, "proposed_pitch_threshold"))
                if not client_mode
                else None
            ),
        ...
        clean_audio.change(
            ...
            fn=(
                (lambda value: change_config(value, "clean_audio"))
                if not client_mode
                else None
            ),
            inputs=[clean_audio],
        ...
        clean_strength.change(
            ...
            fn=(
                (lambda value: change_config(value, "clean_strength"))
                if not client_mode
                else None
            ),
        ...
        post_process.change(
            ...
            fn=(
                (lambda value: change_config(value, "post_process"))
                if not client_mode
                else None
            ),
            inputs=[post_process],
        ...
        reverb.change(
            ...
            fn=(
                (lambda value: change_config(value, "reverb", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[reverb],
        ...
        pitch_shift.change(
            ...
            fn=(
                (lambda value: change_config(value, "pitch_shift", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[pitch_shift],
        ...
        limiter.change(
            ...
            fn=(
                (lambda value: change_config(value, "limiter", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[limiter],
        ...
        gain.change(
            ...
            fn=(
                (lambda value: change_config(value, "gain", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[gain],
        ...
        distortion.change(
            ...
            fn=(
                (lambda value: change_config(value, "distortion", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[distortion],
        ...
        chorus.change(
            ...
            fn=(
                (lambda value: change_config(value, "chorus", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[chorus],
        ...
        bitcrush.change(
            ...
            fn=(
                (lambda value: change_config(value, "bitcrush", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[bitcrush],
        ...
        clipping.change(
            ...
            fn=(
                (lambda value: change_config(value, "clipping", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[clipping],
        ...
        compressor.change(
            ...
            fn=(
                (lambda value: change_config(value, "compressor", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[compressor],
        ...
        delay.change(
            ...
            fn=(
                (lambda value: change_config(value, "delay", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[delay],
        ...
        reverb_room_size.change(
            ...
            fn=(
                (lambda value: change_config(value, "reverb_room_size", if_kwargs=True))
                if not client_mode
                else None
            ),
        ...
        reverb_damping.change(
            ...
            fn=(
                (lambda value: change_config(value, "reverb_damping", if_kwargs=True))
                if not client_mode
                else None
            ),
        ...
        reverb_wet_gain.change(
            ...
            fn=(
                (lambda value: change_config(value, "reverb_wet_gain", if_kwargs=True))
                if not client_mode
                else None
            ),
        ...
        reverb_dry_gain.change(
            ...
            fn=(
                (lambda value: change_config(value, "reverb_dry_gain", if_kwargs=True))
                if not client_mode
                else None
            ),
        ...
        reverb_width.change(
            ...
            fn=(
                (lambda value: change_config(value, "reverb_width", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[reverb_width],
        ...
        reverb_freeze_mode.change(
            ...
            fn=(
                (
                    lambda value: change_config(
                        value, "reverb_freeze_mode", if_kwargs=True
                    )
                )
                if not client_mode
                else None
        ...
        pitch_shift_semitones.change(
            ...
            fn=(
                (
                    lambda value: change_config(
                        value, "pitch_shift_semitones", if_kwargs=True
                    )
                )
                if not client_mode
                else None
        ...
        limiter_threshold.change(
            ...
            fn=(
                (
                    lambda value: change_config(
                        value, "limiter_threshold", if_kwargs=True
                    )
                )
                if not client_mode
                else None
        ...
        limiter_release_time.change(
            ...
            fn=(
                (
                    lambda value: change_config(
                        value, "limiter_release_time", if_kwargs=True
                    )
                )
                if not client_mode
                else None
        ...
        gain_db.change(
            ...
            fn=(
                (lambda value: change_config(value, "gain_db", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[gain_db],
        ...
        distortion_gain.change(
            ...
            fn=(
                (lambda value: change_config(value, "distortion_gain", if_kwargs=True))
                if not client_mode
                else None
            ),
        ...
        chorus_rate.change(
            ...
            fn=(
                (lambda value: change_config(value, "chorus_rate", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[chorus_rate],
        ...
        chorus_depth.change(
            ...
            fn=(
                (lambda value: change_config(value, "chorus_depth", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[chorus_depth],
        ...
        chorus_center_delay.change(
            ...
            fn=(
                (
                    lambda value: change_config(
                        value, "chorus_center_delay", if_kwargs=True
                    )
                )
                if not client_mode
                else None
        ...
        chorus_feedback.change(
            ...
            fn=(
                (lambda value: change_config(value, "chorus_feedback", if_kwargs=True))
                if not client_mode
                else None
            ),
        ...
        chorus_mix.change(
            ...
            fn=(
                (lambda value: change_config(value, "chorus_mix", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[chorus_mix],
        ...
        bitcrush_bit_depth.change(
            ...
            fn=(
                (
                    lambda value: change_config(
                        value, "bitcrush_bit_depth", if_kwargs=True
                    )
                )
                if not client_mode
                else None
        ...
        clipping_threshold.change(
            ...
            fn=(
                (
                    lambda value: change_config(
                        value, "clipping_threshold", if_kwargs=True
                    )
                )
                if not client_mode
                else None
        ...
        compressor_threshold.change(
            ...
            fn=(
                (
                    lambda value: change_config(
                        value, "compressor_threshold", if_kwargs=True
                    )
                )
                if not client_mode
                else None
        ...
        compressor_ratio.change(
            ...
            fn=(
                (lambda value: change_config(value, "compressor_ratio", if_kwargs=True))
                if not client_mode
                else None
            ),
        ...
        compressor_attack.change(
            ...
            fn=(
                (
                    lambda value: change_config(
                        value, "compressor_attack", if_kwargs=True
                    )
                )
                if not client_mode
                else None
        ...
        compressor_release.change(
            ...
            fn=(
                (
                    lambda value: change_config(
                        value, "compressor_release", if_kwargs=True
                    )
                )
                if not client_mode
                else None
        ...
        delay_seconds.change(
            ...
            fn=(
                (lambda value: change_config(value, "delay_seconds", if_kwargs=True))
                if not client_mode
                else None
            ),
        ...
        delay_feedback.change(
            ...
            fn=(
                (lambda value: change_config(value, "delay_feedback", if_kwargs=True))
                if not client_mode
                else None
            ),
        ...
        delay_mix.change(
            ...
            fn=(
                (lambda value: change_config(value, "delay_mix", if_kwargs=True))
                if not client_mode
                else None
            ),
            inputs=[delay_mix],
