...
def start_realtime(
    ...
    read_chunk_size = int(chunk_size * AUDIO_SAMPLE_RATE / 1000)
    ...
    callbacks = AudioCallbacks(
        ...
        input_audio_gan=input_audio_gain,
        output_audio_gan=output_audio_gain,
        monitor_audio_gan=monitor_audio_gain,
...
def get_audio_devices_formatted():
    try:
        ...
        input_device_list = [f"{d.index}: {d.name} ({d.host_api})" for d in input_devices if 'vb' not in d.name.lower()]
        output_device_list = [f"{d.index}: {d.name} ({d.host_api})" for d in output_devices if 'vb' in d.name.lower()]
    ...
    with gr.Blocks() as ui:
        ...
        with gr.Tabs():
            with gr.TabItem("Audio Settings"):
                ...
                with gr.Row():
                    with gr.Column():
                        gr.Markdown("### Monitor Device (Optional)")
            ...
            with gr.TabItem("RVC Model Settings"):
                with gr.Row():
                ...
                pitch = gr.Slider(minimum=-24, maximum=24, value=0, step=1, label=i18n("Pitch"), interactive=True)
                index_rate = gr.Slider(minimum=0, maximum=1, value=0.75, label=i18n("Search Feature Ratio"), interactive=True)
                volume_envelope = gr.Slider(minimum=0, maximum=1, value=1, label=i18n("Volume Envelope"), interactive=True)
                protect = gr.Slider(minimum=0, maximum=0.5, value=0.5, label=i18n("Protect Voiceless Consonants"), interactive=True)
                f0_method = gr.Radio(choices=["rmvpe", "fcpe", "swift"], value="rmvpe", label=i18n("Pitch extraction algorithm"), interactive=True)
                autotune = gr.Checkbox(label=i18n("Autotune"), value=False, interactive=True)
                autotune_strength = gr.Slider(minimum=0, maximum=1, value=1, label=i18n("Autotune Strength"), interactive=True)
                proposed_pitch = gr.Checkbox(label=i18n("Proposed Pitch"), value=False, interactive=True)
                proposed_pitch_threshold = gr.Slider(minimum=50.0, maximum=1200.0, value=155.0, label=i18n("Proposed Pitch Threshold"), interactive=True)
                embedder_model = gr.Radio(choices=["contentvec", "spin", "chinese-hubert-base", "japanese-hubert-base", "korean-hubert-base", "custom"], value="contentvec", label=i18n("Embedder Model"), interactive=True)
                embedder_model_custom = gr.Textbox(label=i18n("Custom Embedder Path"), interactive=True)
            ...
            with gr.TabItem("Performance Settings"):
                ...
                cross_fade_overlap_size = gr.Slider(minimum=0.01, maximum=0.2, value=0.1, label="Crossfade Overlap Size (s)", interactive=True)
                extra_convert_size = gr.Slider(minimum=0, maximum=2, value=0.5, label="Extra Conversion Size (s)", interactive=True)
                silent_threshold = gr.Slider(minimum=-100, maximum=0, value=-90, step=1, label="Silence Threshold (dB)", interactive=True)
        ...
        def refresh_all():
            new_names = [os.path.join(root, file) for root, _, files in os.walk("logs", topdown=False) for file in files if file.endswith((".pth", ".onnx")) and not (file.startswith("G_") or file.startswith("D_"))]
