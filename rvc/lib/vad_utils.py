...
if __name__ == '__main__':
    ...
    # Create a dummy VAD processor - RVC pipeline typically uses 16kHz for Hubert
    vad_processor = VADProcessor(sensitivity_mode=3, sample_rate=16000, frame_duration_ms=30)

    # Test with silence
    silent_chunk = np.zeros(int(16000 * 0.5), dtype=np.float32) # 0.5 seconds of silence
    is_speech_detected = vad_processor.is_speech(silent_chunk)
    print(f"Silence test: Speech detected? {is_speech_detected}") # Expected: False

    # Test with some noise (simulating speech)
    # Create a sine wave for testing
    sr_test = 16000
    duration_test = 0.5  # seconds
    frequency_test = 440  # Hz (A4 note)
    t_test = np.linspace(0, duration_test, int(sr_test * duration_test), endpoint=False)
    speech_sim_chunk = 0.5 * np.sin(2 * np.pi * frequency_test * t_test)
    speech_sim_chunk = speech_sim_chunk.astype(np.float32)

    is_speech_detected = vad_processor.is_speech(speech_sim_chunk)
    print(f"Simulated speech test: Speech detected? {is_speech_detected}") # Expected: True

    # Test with a very short chunk (less than one frame_duration_ms)
    short_speech_chunk = speech_sim_chunk[:int(sr_test * 0.015)] # 15ms, frame is 30ms
    is_speech_detected = vad_processor.is_speech(short_speech_chunk)
    print(f"Short speech test (15ms): Speech detected? {is_speech_detected}")

    short_silent_chunk = silent_chunk[:int(sr_test * 0.015)] # 15ms
    is_speech_detected = vad_processor.is_speech(short_silent_chunk)
    print(f"Short silence test (15ms): Speech detected? {is_speech_detected}")

    # Test with empty chunk
    empty_chunk = np.array([], dtype=np.float32)
    is_speech_detected = vad_processor.is_speech(empty_chunk)
    print(f"Empty chunk test: Speech detected? {is_speech_detected}") # Expected: False

print("vad_utils.py loaded")
