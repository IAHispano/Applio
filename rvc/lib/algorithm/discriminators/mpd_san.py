...
class MultiPeriodDiscriminator(torch.nn.Module):
    ...
    def __init__(self, version: str, use_spectral_norm: bool = False, is_san = False):
        ...
        self.discriminators = torch.nn.ModuleList(
            ...
            + [DiscriminatorP(p, use_spectral_norm=use_spectral_norm, is_san=is_san) for p in periods]
        )
...
class DiscriminatorS(torch.nn.Module):
    ...
    def __init__(self, use_spectral_norm: bool = False, is_san = False):
    ...
    def __init__(
        ...
        if is_san:
            ...
            self.conv_post = norm_f(
                torch.nn.Conv2d(1024, 1, kernel_size=(3, 1), stride=1, padding=(1, 0))
            )
        self.lrelu = torch.nn.LeakyReLU(LRELU_SLOPE)
