...
class AutoPitch:
    ...
    def autopitch(self, model, net_g, sid):
        ...
        euclidean_male = pwise.euclidean_distances(emb_rvc, self.emb_loaded["male"]).mean() - 0.7
        euclidean_female = pwise.euclidean_distances(emb_rvc, self.emb_loaded["female"]).mean() - 1

        # can return 155 or 255 depending on the voice model instead of calculating

        euclidean = (euclidean_male + 1.015) if euclidean_male - euclidean_female > 0.1 else (euclidean_female + 1.715)
