...
class AutoPitch:
    ...
    def __init__(self, vc, rvc_npz_path, emb_npz_path, pitch_guidance=True, version="v1"):
        self.vc = vc
        ...
        self.embedders = load_embedding("contentvec", None).to(self.device).float().eval()
    ...
    def get_hubert_feature(self, model, feats):
        with torch.no_grad():
            ...
            feats = model(feats.to(self.device), output_hidden_states=True) # ["last_hidden_state"]
    ...
    def autopitch(self, model, net_g, sid):
        ...
        euclidean_male = pwise.euclidean_distances(emb_rvc, self.emb_loaded["male"]).mean() - self.male_threshold_adjustment
        euclidean_female = pwise.euclidean_distances(emb_rvc, self.emb_loaded["female"]).mean() - self.female_threshold_adjustment

        # can return 155 or 255 depending on the voice model instead of calculating

        euclidean = (euclidean_male + self.male_euclidean_offset) if euclidean_male - euclidean_female > 0.1 else (euclidean_female + self.female_euclidean_offset)
