"""The RefineGAN decoder as Applio shipped it -- inference only.

Kept verbatim so existing checkpoints still render.  It must not be used for
training: it carries the defects ``generators/refinegan.py`` fixes, and the
two are not weight-compatible (the fixed one owns ``source_gain.*``, which is
how ``synthesizers`` tells them apart).

The defects, each measured:

* ``AntiAliasedUpsample1d``'s predecessor here is ``nn.Upsample(mode="linear")``,
  whose triangular kernel rejects the first image by 1.7-9.6 dB, stamping the
  frame grid into the waveform as a mirrored partial either side of every
  harmonic.
* Every activation is a raw ``leaky_relu``.  The six ``AdaIN`` per stage at
  2 and 8 kHz are where the inharmonic lines come from -- A/B renders put the
  whole artefact there.
* ``upsample_rates`` ascending, so the last residual block synthesises
  everything above ``rolloff * rate[-2] / 2`` from scratch.
"""
...
class LegacyRefineGANGenerator(nn.Module):
    """
