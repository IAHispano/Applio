...
class HiFiGAN_Snake(nn.Module):
    def __init__(
        ...
        self.act_post = SnakeBeta(channel)
    ...
    def forward(self, x, f0, g: Optional[torch.Tensor] = None):
        ...
        if g is not None:
            x = x + self.cond(g)        
        i = 1
        torch.save(x, f'00_before_ups.pt')
        for up, mrf, noise_conv in zip(self.upsamples, self.mrfs, self.noise_convs):
            ...
            torch.save(x, f'01_ups_{i}.pt')
            i +=1
        x = self.act_post(x)
        torch.save(x, '02_after_act_post.pt')
        x = self.conv_post(x)
        torch.save(x, '03_after_conv_post.pt')
        x = torch.tanh(x)
        torch.save(x, '04_after_tanh.pt')
        zzz = input()
        return x
