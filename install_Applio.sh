#!/bin/sh
printf "\033]0;Applio-RVC-Fork - Installer\007"
clear
...
prepare_install() {
    ...
    if [ "$r" = "y" ]; then
        ...
        if [ -n "$(python --version | grep -q 3.)" ]; then
            py=$(which python)
            echo "Using python"
        else
...
main_menu() {
    while true; do
        ...
        printf "Select the option according to your GPU: "
        read -r choice
